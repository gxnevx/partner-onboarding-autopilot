  ...
  const response = UrlFetchApp.fetch('https://api.openai.com/v1/responses', {
    ...
    payload: JSON.stringify({
      ...
      instructions: [
        ...
        'Match the supplied brand voice. Keep each email practical, credible, and between 80 and 150 words.',
        'Return plain-text email bodies with natural paragraph breaks. Do not use Markdown.',
      ].join(' '),
  ...
  const response = UrlFetchApp.fetch('https://api.openai.com/v1/responses', {
    ...
    payload: JSON.stringify({
      ...
      instructions: [
        ...
        'Use only facts, commercial terms, contacts, and URLs supplied in the brief.',
        'Preserve every supplied URL exactly. Never invent a link, feature, claim, or customer result.',
        'Match the supplied brand voice. Keep the email practical, credible, and between 80 and 150 words.',
        'Return a plain-text body with natural paragraph breaks. Do not use Markdown.',
      ].join(' '),
      input: JSON.stringify(strategicBrief),
      text: {
        format: {
